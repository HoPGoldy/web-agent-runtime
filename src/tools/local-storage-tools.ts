...
function createReadTool(resolveStorage: () => Storage, keyPrefix: string): LocalStorageTool {
  return {
    ...
    inputSchema: {
      ...
      properties: {
        key: {
          type: "string",
          description: "Optional key to read. Omit to list all available keys and values.",
        },
      },
    ...
    async execute({ input }) {
        ...
        return createToolResult(
          totalEntries.length === 0
            ...
            : [
                ...
                ...totalEntries.map((entry) => `- ${entry.key} = ${JSON.stringify(entry.value)}`),
              ].join("\n"),
          {
            operation: "read",
            keyPrefix,
      ...
      return createToolResult(
        exists
          ? [`Key "${readInput.key}" exists.`, `Value:\n${formatValue(value)}`].join("\n\n")
...
function createCreateTool(resolveStorage: () => Storage, keyPrefix: string): LocalStorageTool {
  return {
    ...
    async execute({ input }) {
      ...
      return createToolResult(
        [`Created key "${createInput.key}".`, `Value:\n${formatValue(createInput.value)}`].join("\n\n"),
        {
          operation: "create",
          key: createInput.key,
...
function createUpdateTool(resolveStorage: () => Storage, keyPrefix: string): LocalStorageTool {
  return {
    ...
    async execute({ input }) {
      ...
      return createToolResult(
        [`Updated key "${updateInput.key}".`, `Value:\n${formatValue(updateInput.value)}`].join("\n\n"),
        {
          operation: "update",
          key: updateInput.key,
...
function createDeleteTool(resolveStorage: () => Storage, keyPrefix: string): LocalStorageTool {
  return {
