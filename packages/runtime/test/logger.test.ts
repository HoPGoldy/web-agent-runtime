...
describe("logger", () => {
    ...
    expect(
      entries.some(
        (entry) =>
          ...
          entry.message.includes("runtime:prompt:start"),
      ),
    ).toBe(true);
    expect(
      entries.some((entry) =>
        entry.message.includes("storage:indexeddb:create-session:start"),
      ),
    ).toBe(true);
  });
  ...
  it("filters verbose logs but preserves warnings when logLevel is warning", async () => {
    ...
    expect(
      entries.some((entry) => entry.message.includes("runtime:prompt:start")),
    ).toBe(false);
    expect(
      entries.some(
        (entry) =>
          entry.level === LogLevel.Warning &&
          entry.message.includes("loop:tool-call:missing-tool"),
      ),
    ).toBe(true);
  });
});
