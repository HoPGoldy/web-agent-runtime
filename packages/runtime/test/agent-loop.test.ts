...
describe("agent loop semantics", () => {
  it("injects steering after tool execution and before the next model call", async () => {
    ...
    const tools = createStaticTools([
      {
        ...
        async execute({ signal }) {
          ...
          await runtimeRef.current?.steer("steered");
          return {
    ...
    expect(requests[1]?.messages.map(readUserText).filter(Boolean)).toContain(
      "steered",
    );
  });
  ...
  it("delivers follow-up only after the agent would otherwise stop", async () => {
    ...
    const tools = createStaticTools([
      {
        ...
        inputSchema: { type: "object" },
        async execute() {
          await runtimeRef.current?.followUp("queued follow-up");
          return {
            content: [{ type: "text", text: "tool result" }],
          };
        },
    ...
    expect(
      requests[1]?.messages.map(readUserText).filter(Boolean),
