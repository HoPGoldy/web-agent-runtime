...
import type {
  IndexedDbAgentStorageOptions,
  StoredOpaqueSessionData,
} from "../types/storage";
import {
  createRuntimeLogger,
  traceRuntimeDebug,
...
> implements StorageProvider<TSessionData> {
  ...
  async createSession(input: CreateSessionInput = {}) {
    ...
    traceRuntimeDebug(
      this.logger,
      "storage:indexeddb:create-session:db-ready",
      {
        dbName: this.dbName,
    ...
    traceRuntimeDebug(
      this.logger,
      "storage:indexeddb:create-session:put-dispatched",
      {
        sessionId: session.id,
      },
    );
    ...
    if (!this.dbPromise) {
      this.dbPromise = new Promise<IDBDatabase>((resolve, reject) => {
        ...
        request.onupgradeneeded = () => {
          traceRuntimeDebug(
            this.logger,
            "storage:indexeddb:get-db:open:upgrade-needed",
            {
              dbName: this.dbName,
            },
          );
        ...
        request.onblocked = () => {
          traceRuntimeWarning(
            this.logger,
            "storage:indexeddb:get-db:open:blocked",
            {
              dbName: this.dbName,
            },
          );
        ...
        request.onsuccess = () => {
          traceRuntimeDebug(
            this.logger,
            "storage:indexeddb:get-db:open:success",
            {
              dbName: this.dbName,
            },
          );
        ...
        request.onerror = () => {
          traceRuntimeError(
            this.logger,
            "storage:indexeddb:get-db:open:error",
            {
              dbName: this.dbName,
