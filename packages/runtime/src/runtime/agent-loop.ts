import type { ToolDefinition, ToolExecutionResult } from "../types/provider";
import type { AgentLoopBindings, PromptInput } from "../types/runtime";
import type {
  AgentMessage,
  AssistantMessage,
  ToolCallBlock,
...
export class AgentLoopEngine<THostContext = unknown> {
    ...
    try {
      while (true) {
        ...
        if (pendingMessages.length > 0) {
          traceRuntimeDebug(
            this.bindings.logger,
            "loop:pending-messages:append:start",
            {
              turnCount,
              pendingMessageCount: pendingMessages.length,
            },
          ...
          traceRuntimeDebug(
            this.bindings.logger,
            "loop:pending-messages:append:done",
            {
              turnCount,
            },
          );
  ...
  private async runTurn(signal: AbortSignal) {
    ...
    if (!assistantMessage) {
      traceRuntimeDebug(
        this.bindings.logger,
        "loop:runTurn:stream-result:start",
      );
      assistantMessage = (await stream.result()) as AssistantMessage;
      traceRuntimeDebug(
        this.bindings.logger,
        "loop:runTurn:stream-result:done",
        {
    ...
    traceRuntimeDebug(
      this.bindings.logger,
      "loop:runTurn:append-assistant:start",
    );
    await this.bindings.appendMessages([assistantMessage]);
    traceRuntimeDebug(
      this.bindings.logger,
      "loop:runTurn:append-assistant:done",
    );
