    ...
    stream: {
      async *[Symbol.asyncIterator]() {
        while (true) {
          ...
          const item = await new Promise<IteratorResult<TEvent>>(
            (resolve, reject) => {
...
function buildDefaultRequestBody(
  ...
  return {
    ...
    stream: true,
    stream_options:
      options.includeUsage === false ? undefined : { include_usage: true },
  };
}
...
function toAssistantMessage(
  ...
  const toolCalls = (choice.message.tool_calls ?? []).map<ToolCallBlock>(
    (toolCall) => ({
      type: "toolCall",
      id: toolCall.id,
...
function buildAssistantEvents(message: AssistantMessage) {
  ...
  for (const block of message.content) {
    ...
    if (block.type === "text") {
      ...
      (partial.content[contentIndex] as { type: "text"; text: string }).text =
        block.text;
      events.push({
        type: "text_delta",
        contentIndex,
...
function ensureTextBlock(
  ...
  queue: ReturnType<
    typeof createQueuedResultStream<
...
function appendTextDelta(
  ...
  const block = accumulator.partial.content[contentIndex] as {
    type: "text";
    text: string;
  };
  block.text += delta;
...
function appendToolCallDeltas(
  ...
  deltas.forEach((delta, arrayIndex) => {
    ...
    const block = accumulator.partial.content[
      entry.contentIndex
    ] as ToolCallBlock;
    if (delta.id) {
      block.id = delta.id;
...
function finalizeTextBlock(
  ...
  const block = accumulator.partial.content[accumulator.textIndex] as {
    type: "text";
    text: string;
  };
  queue.push({
    ...
    async stream(request) {
      ...
      if (!isEventStreamResponse(response)) {
        ...
        return createResultStream(
          buildAssistantEvents(assistantMessage),
          assistantMessage,
        );
      }
