...
const DEFAULT_PROMPT =
  '把 key profile 保存成 JSON，内容是 {"name":"Wesley","city":"Shanghai" }。';
...
export default function LocalStorageChatApp() {
  ...
  const [bootStatus, setBootStatus] = useState<BootStatus>(
    OPENAI_KEY ? "idle" : "config-missing",
  );
      ...
      <section className="chat-frame">
        ...
        <div className="composer-shell">
          ...
          <textarea
            ...
            spellCheck={false}
            placeholder={
              '例如：创建一个 key 叫 tasks，值是 ["ship demo", "write docs"]'
            }
          />
          <div className="composer-actions">
            ...
            <button
              ...
              disabled={runtimeState?.status === "streaming"}
            >
              清空 demo data
            </button>
            <button
  ...
  switch (message.role) {
    case "user":
      return typeof message.content === "string"
        ? message.content
        : stringifyContentBlocks(message.content);
    case "assistant": {
...
function stringifyContentBlocks(
