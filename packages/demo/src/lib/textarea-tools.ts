...
function createReadTool(
  ...
  return {
    name: "textarea_read",
    description:
      "读取文本框当前内容、长度和选区，编辑前如果不确定上下文先调用这个工具。",
    inputSchema: {
      type: "object",
      properties: {
        includeText: {
...
function createInsertTool(
  ...
  return {
    ...
    description:
      "在文本框插入新内容。默认插入到当前光标位置，也可以显式指定 index。",
    inputSchema: {
      type: "object",
      properties: {
        text: {
    ...
    async execute({ input }) {
      ...
      const insertionPoint = clampIndex(
        input.index ?? current.selection.start,
        current.length,
      );
      const nextValue =
        current.value.slice(0, insertionPoint) +
        input.text +
        current.value.slice(insertionPoint);
...
function createUpdateTool(
  ...
  return {
    ...
    description:
      "替换文本框中的一段内容。未提供 range 时优先替换当前选区，若没有选区则替换整段文本。",
    inputSchema: {
      type: "object",
      properties: {
        text: {
    ...
    async execute({ input }) {
      ...
      const nextValue =
        current.value.slice(0, normalizedStart) +
        input.text +
        current.value.slice(normalizedEnd);
      const snapshot = target.apply(nextValue, {
...
function createDeleteTool(
  ...
  return {
    ...
    description:
      "删除文本框中的一段内容。默认删除当前选区；传入 mode=all 时清空整个文本框。",
    inputSchema: {
      type: "object",
      properties: {
        start: {
    ...
    async execute({ input }) {
      ...
      if (input.mode === "all") {
        ...
        const current = target.read();
        const rangeStart = clampIndex(
          input.start ?? current.selection.start,
          current.length,
        );
        const rangeEnd = clampIndex(
          input.end ?? current.selection.end,
          current.length,
        );
        const normalizedStart = Math.min(rangeStart, rangeEnd);
        const normalizedEnd = Math.max(rangeStart, rangeEnd);
