...
**额外约束：**
6. ✅ 采用 TDD - 每个功能切片先写失败测试，再写实现
7. ✅ 不确定行为优先参考 `pi-mono/packages/agent` 和 `pi-mono/packages/coding-agent`
...
- [ ] **T-010**: 先写失败测试，定义 runtime 内部 session graph 与 codec 行为
  - **复杂度**: 中
...
- [ ] **T-013**: 实现新的 storage provider 契约和默认浏览器适配
  - **复杂度**: 中
...
- [ ] **T-030**: 补齐回归测试，覆盖 revision 冲突、codec 失败、tool abort 和非法 continue
  - **复杂度**: 中
...
|--------|----------|----------|--------|------|----------|
| 自动 compaction 是否进入 v1 范围待确认 | T-020, T-021 | 2026-04-09 | User | 待处理 | 规格确认后决定 |
