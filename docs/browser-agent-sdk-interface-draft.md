...
- 在浏览器本地执行
- 通过后端代理访问模型
- 通过 provider 接口接入 LLM、storage 和 tool
- v1 支持 session CRUD、session fork、compaction、steering、follow-up 和 tool execution

## 阶段 1 导出基线

为了在不破坏现有公开 API 的前提下启动迁移，第一阶段先固定一组过渡期导出边界：

- 根入口继续保留现有 `Agent`、tool factory、`IndexedDbAgentStorage` 等导出
- 新增三个 namespaced contract export：`runtimeContracts`、`sessionContracts`、`providerContracts`
- 这三个 namespace 在第一阶段只承载类型契约和占位声明，不包含行为实现
- 等核心实现阶段完成后，再评估是否将这些 contract 扁平化为最终 root exports

这样可以先冻结 SDK 目标边界，再逐步替换旧实现，而不需要在第一阶段立即删除老代码
...
export interface ToolProviderContext<THostContext = unknown> {
  session: SessionRecord | null;
  state: RuntimeState;
  hostContext: THostContext;
}

export interface ToolProvider<THostContext = unknown> {
  getTools(
    context: ToolProviderContext<THostContext>,
  ): Promise<ToolDefinition<unknown, unknown, THostContext>[]>;
}
```
...
export interface SessionDataCodec<TSessionData = unknown> {
...
export interface RuntimeState {
  ...
  queuedSteeringMessages: UserMessage[];
  queuedFollowUpMessages: UserMessage[];
  error?: string;
}
...
```ts
export type PromptInput = string | UserMessage | UserMessage[];
...
export interface AgentRuntimeOptions<THostContext = unknown, TSessionData = RuntimeSessionData> {
  ...
  toolProvider: ToolProvider<THostContext>;
  systemPrompt?: string;
...
这份草案确认后，下一份文档应当定义从当前 `web-agent-runtime` 演进到这套接口的具体迁移方案，至少包括：

- 哪些内容需要删除
- 哪些内容会保留为 adapter
- 哪些能力应该从 `packages/agent` 迁移过来
- 哪些能力应该有选择地从 `packages/coding-agent` 迁移过来
  \*\*\* End Patch
